...
import CopyMeLogic from "./CopyMeLogic";

export const COPYME_VERSION = "v1.0.1";

const CopyMe = () => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const logicRef = useRef<CopyMeLogic | null>(null);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;

    logicRef.current = new CopyMeLogic(canvas);
  ...
  return (
    <div className="flex-1 w-full overflow-hidden flex flex-col">
      <div className="bg-base-200 flex-none flex items-center justify-between h-12 px-4 pl-12 sm:pl-4">
        <h1 className="text-lg">Generate Maze {COPYME_VERSION}</h1>
      </div>
