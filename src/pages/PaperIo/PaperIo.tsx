...
import PaperIoLogic from "./PaperIoLogic";

const PaperIo = () => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const logicRef = useRef<PaperIoLogic | null>(null);
  ...
  useEffect(() => {
    ...
    logicRef.current = new PaperIoLogic(canvas);
  ...
  return (
    <div className="flex-1 w-full overflow-hidden flex flex-col">
      <div className="bg-base-200 flex-none flex items-center justify-between h-12 px-4 pl-12 sm:pl-4">
        <h1 className="text-lg">Paper Io</h1>
      </div>
